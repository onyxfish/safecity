...
Due to the sensitive nature of the information handled by this project extreme care is taken to protect the privacy of its users.  The explicit standard of SafeCity is that the identity of its users should will never be available to a third-party, even if the contents of the database are subpoenaed.  This is accomplished in a number of ways, most importantly:
...
* Users who unregister from the system are deleted--no record is kept that they were ever registered with the system.